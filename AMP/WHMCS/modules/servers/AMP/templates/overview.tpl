   ...
   <div class="modal-dialog" style="max-width:500px;">
      <div class="modal-content">
         <div class="modal-header">
            ...
            <h4 class="modal-title" id="modalLabelLarge">Vahvista toiminto</h4>
         </div>
...
<span id="actionMessage"></span>
         ...
         <div class="modal-footer">
            <button type="button" class="btn btn-secondary bbtn" data-dismiss="modal">Sulje</button>
            <button  id="confirm" type="button" class="btn btn-success ">Jatka</button>
         </div>
      </div>
   </div>
</div>
...
<div class="panel panel-primary">
    <div class="panel-heading"><h4>Pelipalvelimen ohjauspaneeli</h4></div>
    <div class="panel-body">
        <div class="row">
            <div class="col-sm-8">
                <p>Hallinnoi pelipalvelinkonsoliasi, jotta voit hallita monenlaisia toimintoja, mukaan lukien asetusten, lisäosien, modien, varmuuskopioiden ja monia muita ominaisuuksia.</p>
            </div>
        ...
        <div class="alert alert-info">
            <strong>Vinkki:</strong> Jos et pääse käsiksi konsoliisi, varmista, että Palvelimesi  on käynnissä alla.
        </div>
    </div>
</div>
            ...
            <div class="panel-body">
                {foreach $endpoints as $e}
                    ...
                    {else}
                        {$e['Endpoint']}
                    {/if}
                    <button style="margin-left: 16px;" onclick="copyToClipboard('{$cleanAppUrl}:{$parts[1]}')" class="btn btn-default">Kopio leikepöydälle</button>
                    <hr>
                {/foreach}
            </div>
        </div>
            ...
            <div class="panel-body">
                {foreach $endpoints as $e}
                    ...
                    {else}
                        {$e['Endpoint']}
                    {/if}
                    <button style="margin-left: 16px;" onclick="copyToClipboard('{$e['Endpoint']}')" class="btn btn-default">Kopio Leikepöydälle</button>
                    <hr>
                {/foreach}
            </div>
        </div>
...
<div class="panel panel-default">
    <div class="panel-heading"><h4>Palvelimen Hallinta</h4></div>
    <div class="panel-body">
        <h5>Tila: <span id="status"></span></h5>
        <div class="text-sm-left text-center">
            <button id="start" class="btn btn-success" data-toggle="modal" data-target="#confirmModal" style="min-width: 150px; margin-top: 2px;">Käynnistä palvelin</button>
            <button id="stop" class="btn btn-danger" data-toggle="modal" data-target="#confirmModal" style="min-width: 150px; margin-top: 2px;">Sammuta palvelin</button>
            <button id="restart" class="btn btn-warning" data-toggle="modal" data-target="#confirmModal" style="min-width: 150px; margin-top: 2px;">Uudelleen käynnistä</button>
            <button id="resetPassword" class="btn btn-info" data-toggle="modal" data-target="#confirmModal" style="min-width: 150px; margin-top: 2px;">Resetoi salasana</button>
        </div>
    </div>
</div>
{/if}
